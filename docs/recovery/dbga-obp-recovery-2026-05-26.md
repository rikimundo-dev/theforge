...
```
### Esquema SQL para tablas espejo (OBP4MO)
```sql
...
-- Se asocian a país (los formatos varían por país)
...
### Esquema SQL para tablas espejo (OBP — estructura desnormalizada)
```sql
...
CREATE TABLE ubicaciones_obp (
  ...
  pais_referencia VARCHAR(255), -- campo libre mientras no se normalice
...
CREATE TABLE formatos_medio_obp (
  ...
  siglas VARCHAR(10),
  descripcion TEXT,
  ubicacion_id UUID REFERENCES ubicaciones_obp(id), -- mientras no haya país explícito
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
**Contexto:** Este módulo será una tabla muy simple con tabs para los medios que muestren columnas definidas con la única acción de modificar el costo de un medio.
...
flowchart TD
  start("Inicio")
  p0("1. Odoo genera una OC con costos reales.")
  start --> p0
  p1("2. Odoo envía el payload al endpoint ` api v1 od")
  p0 --> p1
  p2("3. El microservicio valida el API key.")
  p1 --> p2
  p3("4. Busca el registro en `campaign_media_costs` q")
  p2 --> p3
  p4("5. Si existe:")
  p3 --> p4
  p5("Actualiza `actual_amount` con el valor recibido.")
  p4 --> p5
  p6("Calcula el nuevo margen y actualiza `margin_stat")
  p5 --> p6
  p7("Registra el cambio en `audit_logs`.")
  p6 --> p7
  p8("Si el margen cambia de estado, dispara notificac")
  p7 --> p8
  p9("6. Si no existe:")
  p8 --> p9
  p10("Crea un nuevo registro en `campaign_media_costs`")
  p9 --> p10
  p11("Marca `odoo_sync_status = 'SYNCED'`.")
  p10 --> p11
  p12("7. Responde con el resultado.")
  p11 --> p12
  dec{"Registro existe?"}
  p12 --> dec
  finish("Respuesta API")
  dec --> finish
```

- 1. Odoo genera una OC con costos reales.
- 2. Odoo envía el payload al endpoint `/api/v1/odoo/costos-reales`.
- 3. El microservicio valida el API key.
- 4. Busca el registro en `campaign_media_costs` que coincida con `campaign_id` + `media_id` + `cost_type`.
- 5. Si existe:
- Actualiza `actual_amount` con el valor recibido.
- Calcula el nuevo margen y actualiza `margin_status`.
- Registra el cambio en `audit_logs`.
- Si el margen cambia de estado, dispara notificaciones según corresponda.
- 6. Si no existe:
- Crea un nuevo registro en `campaign_media_costs` con `actual_amount`.
- Marca `odoo_sync_status = 'SYNCED'`.
- 7. Responde con el resultado.
