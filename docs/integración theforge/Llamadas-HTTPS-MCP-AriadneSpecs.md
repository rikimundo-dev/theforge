...
| `ask_codebase`                  | `question`              | `projectId`, `currentFilePath`, `scope`, `twoPhase`, **`responseMode`** (`default` \| **`evidence_first`**) — ver `mcp_server_specs` (`evidence_first`: two-phase + más contexto al sintetizador, salida “evidencia primero”) |
...
- **Contrato de argumentos:** `theforge-mcp-client-contract.ts` + `pnpm --filter @theforge/api test:mcp-alignment` (herramientas esperadas y claves frente a `tools/list`).
