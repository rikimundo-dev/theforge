...
- **Estimación MXN:** `src/utils/costCalculator.ts` delega en `@theforge/business-rules` (misma lógica que el API). Vite resuelve el paquete al **fuente** del monorepo (`vite.config.ts` + `tsconfig` paths) para que Rollup no falle con re-exports CJS del `dist` (incl. `@theforge/shared-types/mdd-governance-patterns` para el wizard SSOT).
