...
| **MddPatternsWizardDialog** | Selector SSOT por pestañas (`initial \| edit`): 6 categorías de patrones con badge de selección por tab. Antes de abrir: `POST …/mdd/suggest-governance-patterns` (DBGA, benchmark, BRD). Al confirmar: MDD solo con patrones `[X]` + `POST …/mdd/record-governance-pattern-adrs`. |
