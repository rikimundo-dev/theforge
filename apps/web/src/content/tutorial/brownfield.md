...
**Auto-wire brownfield converge:** si `THEFORGE_MCP_URL` está activo, la API hace `PATCH` en Ariadne al **crear** legacy (etapa 1) y al **promover integración → nueva etapa** (actualiza `theforgeStageId` a la etapa promovida). Requiere token MCP (`MCP_AUTH_TOKEN` o token en Ajustes → Ariadne). Desactivar: `ARIADNE_BROWNFIELD_CONVERGE_AUTO=0`.
