...
|- **Tabla resumen:** Tabla Markdown estandar con `|` pipes. El pipeline normaliza automáticamente el padding, líneas en blanco tras separador y alignment. PROHIBIDO usar viñetas en vez de pipes.
|- **NO Swagger/OpenAPI ni esquemas de documentación automatizada:** No generes objetos OpenAPI, ni `openapi: 3.0.0`, ni `paths:`, ni `components/schemas`. La seccion 4 debe ser markdown legible por humanos.
