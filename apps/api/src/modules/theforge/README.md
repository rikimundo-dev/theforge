...
**`ask_codebase` extendido (Ariadne SPEC):** `AskCodebaseOptions` admite `responseMode: raw_evidence` y `deterministicRetriever: true` (retrieve sin LLM cuando el ingest lo soporta). El cliente **no** normaliza JSON en `raw_evidence`; el caller debe interpretar la respuesta. Uso típico en The Forge: `evidence_first` vía `getLegacyAskCodebaseOptions()` (activable con `LEGACY_ASK_CODEBASE_EVIDENCE_FIRST`, default activo).
