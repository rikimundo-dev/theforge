...
**MDD con JSON crudo pegado por el LLM:** el descubrimiento escalonado a veces vuelca en el markdown un objeto `{ "mode": "raw_evidence", … }` aunque la herramienta ya devolviera texto normalizado. Antes de persistir `legacyFlowState.codebaseDoc`, `LegacyCoordinatorService.generateCodebaseDocCore` aplica **`normalizeRawEvidenceJsonBlocksInMarkdown`**: detecta esos objetos JSON, los parsea y los sustituye por markdown con tablas de conteos/muestras, `collectedResults`, `cypher`, etc. Por defecto **no** se trunca ese markdown; caps opcionales vía `RAW_EVIDENCE_*` (entero > 0) si un despliegue necesita acotar memoria/prompt.
