...
- **`staged-discovery-mdd-prompt.md`** — System prompt Plan-and-Execute para MDD inicial / evidencia de cambio. Placeholders:
  ...
  El flujo obliga **Fase 0** (repos y roles) antes de búsquedas masivas. El contrato técnico del prompt dirige a **`get_contract_specs`** / **`get_implementation_details`** cuando ya hay nombre de componente o símbolo (evitar solo `semantic_search` genérico).
