...
- `POST /projects/:projectId/legacy/generate-mdd` — Genera el MDD y persiste en `mddContent`. **Etapa 1 (`ordinal === 1`):** MDD **AS-IS** del sistema completo — no inyecta preámbulo BRD ni consultas de cambio aunque exista `description` en `legacyChangeState`; §1 Contexto describe el sistema **tal como existe hoy** (prohibido lenguaje de modificación/MVP). Tras la revisión, **§3 y §4 se sustituyen** por las tablas completas de entidades y contratos API del `codebaseDoc` (Ariadne) — evita resúmenes LLM tipo «Otras entidades (N+ adicionales)». Desactivar: `LEGACY_AS_IS_MDD_EVIDENCE_INJECT=0`. Inyecta **`### Diagrama de Componentes`** en §2 si falta. **Etapas 2+:** MDD de **cambio** con preámbulo BRD, línea base de la etapa anterior y descripción del cambio. Usa descubrimiento escalonado TheForge + revisor.
