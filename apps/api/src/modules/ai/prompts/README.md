...
| **brd-generation-prompt.md**          | System + plantilla de secciones BRD «firmable» (RF, RNF, RACI, flujos, contratos preliminares, decision log). `buildBrdUserPrompt()` en `suggest-brd-from-dbga` y `legacy/suggest-brd-from-codebase-doc`.                                                                                  |
